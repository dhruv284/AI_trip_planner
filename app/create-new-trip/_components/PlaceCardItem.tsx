...
function PlaceCardItem({activity}:Props) {
  const [photoUrl,setPhotoUrl]=useState<string>()
    ...
    const GetGooglePlaceDetail=async ()=>{
      const result= await axios.post('/api/google-place-detail',{
          textQuery:activity?.place_name+":"+activity?.place_address
      })
      if (result?.data?.e){
        return
      }
  ...
  return (
    <div >
        ...
        <h2 className='flex gap-2 text-blue-500 line-clamp-2'><Ticket/>{activity.ticket_pricing}</h2>
        ...
        <Button size={'sm'} variant={'outline'} className='w-full mt-2'>View <ExternalLink></ExternalLink></Button>
        </Link>
    </div>
  )
}
